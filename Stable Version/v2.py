...
BOX_WIDTH = 450
BOX_HEIGHT = 350
API_KEY = '7xh4R2m83849TzGPdEhF6Bbc'  # Replace with your remove.bg API key
...
class PassportPhotoApp:
    def __init__(self, window):
        ...
        # Frame for captured image preview (right side) - set same size as left_frame
        self.right_frame = Frame(self.container_frame, bd=2, relief="groove", bg="white", width=BOX_WIDTH, height=BOX_HEIGHT)
        self.right_frame.grid(row=0, column=1, padx=20, pady=10)  # Match the padding for uniformity
        ...
        self.account_limit_button = Button(self.button_frame, text="Account Limit", font=("Arial", 12), command=self.check_account_limit, bg="#FF9800", fg="white", width=15, height=1)
        self.account_limit_button.pack(pady=10)
    ...
    def remove_background(self):
        if self.photo:
            ...
            if response.status_code == requests.codes.ok:
                ...
                messagebox.showerror("Error", "Failed to remove background: " + response.text)
    ...
    def check_account_limit(self):
        # Request account balance and API usage
        ...
        if response.status_code == 200:
            ...
            messagebox.showinfo("Account Limit", f"Total Credits: {credits['total']}\n"
                                                  f"Subscription Credits: {credits['subscription']}\n"
                                                  f"Pay-As-You-Go Credits: {credits['payg']}\n"
                                                  f"Free Calls Remaining: {api_info['free_calls']}")
        else:
            print("Error:", response.status_code, response.text)
            messagebox.showerror("Error", "Failed to retrieve account limit: " + response.text)
